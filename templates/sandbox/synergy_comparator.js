...
function inferPresetTypeName(items, hero, desc, idx) {
  ...
  if (desc.includes('全输出') || desc.includes('直接击破') || desc.includes('秒杀')) return '极限高爆秒人';
  if (desc.includes('全肉') || desc.includes('提高生存') || desc.includes('控制敌人')) return '全肉控制抗伤';
  if (desc.includes('半肉') || desc.includes('容错') || desc.includes('吸血')) return '半肉稳健容错';
  if (desc.includes('穿透') || desc.includes('暴击')) return '暴击破甲输出';
  if (desc.includes('冷却') || desc.includes('频繁的使用')) return '极速冷却拉扯';

  // 2. 根据装备数值属性智能定性
  if (role.includes('射手')) {
    if (crit >= 35 || names.includes('破军')) return '暴击穿透秒人';
    if (hp >= 1500 || names.includes('纯净苍穹') || names.includes('暴烈之甲')) return '半肉自保流';
    return idx === 0 ? '暴击破甲流' : '法球攻速流';
  }

  if (role.includes('法师')) {
    if (ap >= 600 || names.includes('博学者之怒') || names.includes('贤者之书')) return '法核爆发秒人';
    if (names.includes('痛苦面具') || names.includes('凝冰之息') || names.includes('噬神之书')) return '冷却消耗拉扯';
    return idx === 0 ? '法核爆发秒人' : '技能消耗拉扯';
  }

  if (role.includes('坦克') || role.includes('辅助')) {
    if (ad <= 80 && hp >= 4000) return '全肉承伤控场';
    if (ad >= 100 || names.includes('暗影战斧') || names.includes('暴烈之甲')) return '半肉进场输出';
    return idx === 0 ? '重装坦伤控场' : '半肉输出对拼';
  }

  if (role.includes('刺客')) {
    if (ad >= 240 || names.includes('破军')) return '瞬杀高爆收割';
    if (hp >= 1500 || names.includes('纯净苍穹') || names.includes('名刀·司命')) return '半肉进场容错';
    return idx === 0 ? '极致爆发秒人' : '半肉收割容错';
  }

  // 战士/对抗路
  if (hp >= 2000 && ad >= 140) return '半肉战阵突进';
  if (hp >= 3500) return '重装坦伤反击';
  if (ad >= 250) return '高爆刺杀流';
  return idx === 0 ? '半肉稳健进场' : '极限输出切入';
}
...
function getHeroOfficialPresets(hero) {
  ...
  if (typeof OFFICIAL_HERO_BUILDS !== 'undefined' && OFFICIAL_HERO_BUILDS[cname]) {
    ...
    return rawList.map((p, idx) => {
      ...
      return {
        ...
        title: typeTitle, // 如“全肉抗伤”、“半肉输出”、“高爆发秒人”
        tag: `官方方案${idx + 1}`,
        desc: p.desc || '王者官方推荐经典实战思路。',
  ...
  // 2. 兜底 fallback (按职业划分经典双方案)
  const role = hero.role || '';
  const lane = hero.lane || '';
  const r = role + lane;

  let p1Names = [], p2Names = [];
  let p1Desc = '官方综合胜率最高的经典高爆配装思路。';
  let p2Desc = '强化实战对拼与防御容错的稳健方案。';
  ...
  return [
    { id: 'official_1', title: inferPresetTypeName(p1Items, hero, p1Desc, 0), tag: '官方方案1', desc: p1Desc, itemNames: p1Names, items: p1Items },
    { id: 'official_2', title: inferPresetTypeName(p2Items, hero, p2Desc, 1), tag: '官方方案2', desc: p2Desc, itemNames: p2Names, items: p2Items }
  ];
}
