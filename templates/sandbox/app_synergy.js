...
function renderSynergyContent() {
  ...
  // 4. 多维度真实契合度评分与动态雷达系统 (拒绝全99分假数值，彻底还原物理实战契合度)
  ...
  // 强击唯一被动冲突 (宗师/冰痕/巫术/光辉同时出只能触发一个强击)
  const spellbladeCount = effItems.filter(it => ['宗师之力', '冰痕之握', '巫术法杖', '光辉之剑'].includes(it.item_name)).length;
  const hasSpellbladeConflict = spellbladeCount > 1;

  // A. 品阶与成型度分 (0 ~ 30分)
  const tierScore = (tier3Count * 5) + (tier2Count * 2.5) + (tier1Count * 1);
  const emptySlotPenalty = Math.max(0, 6 - currentSlots.length) * 4;

  // B. 英雄专属机制与被动高光联动分 (0 ~ 30分)
  let synergyBonus = 0;
  ...
  if (isPhysicalHero) {
    if (heroRole.includes('战士')) {
      if (totalBonusHp >= 2000 && (totalAd >= 100 || hasYellowShield)) balanceScore = 15;
      else if (totalBonusHp === 0 && totalPdef === 0) balanceScore = 5;
    } else {
      if (totalAd >= 220 || totalCrit >= 40 || totalPhysPierce >= 150) balanceScore = 15;
    }
  } else if (isMagicHero) {
    if (totalAp >= 450) balanceScore = 15;
  } else if (isTankOrSupport) {
    if (totalBonusHp >= 2500 && (totalPdef + totalMdef >= 500)) balanceScore = 15;
  }
  ...
  if (wastedApCount > 0) {
    penalties += wastedApCount * 15;
    penaltyReasons.push(`物法错位：物理英雄出了 ${wastedApCount} 件法术装备，法强无法转化有效输出`);
  }
  if (wastedAdCount > 0) {
    penalties += wastedAdCount * 15;
    penaltyReasons.push(`物法错位：法术英雄出了 ${wastedAdCount} 件物理武器，攻击暴击收益严重浪费`);
  }
  if (bootsCount > 1) {
    penalties += (bootsCount - 1) * 15;
    penaltyReasons.push(`重复出鞋：装备了 ${bootsCount} 双鞋子，唯一移速被动无法叠加`);
  }
  if (duplicateItemCount > 0) {
    penalties += duplicateItemCount * 12;
    penaltyReasons.push(`大件重复：装配了 ${duplicateItemCount} 件同名大件，唯一被动全部被浪费`);
  }
  if (hasSpellbladeConflict) {
    penalties += 8;
    penaltyReasons.push('强击冲突：宗师/冰痕/巫术的【强击】被动互斥，同时生效受限');
  }
  if (tier1Count >= 4) {
    penalties += 12;
    penaltyReasons.push('散件过多：装备栏主要由初级散件构成，缺少关键成装被动');
  }

  // 计算综合得分 (最低 25，最高 99)
  let rawScore = 25 + tierScore + synergyBonus + balanceScore - emptySlotPenalty - penalties;
  const score = Math.max(25, Math.min(99, Math.round(rawScore)));

  // 动态等级徽章与评语
  let rankBadge = '', rankColor = '', rankSub = '';
  if (score >= 95) {
    rankBadge = 'S+ 卓越级战术协同';
    rankColor = '#34c759';
    rankSub = '被动特效与英雄动作链高度共鸣，攻防周转达到版本天花板最优解';
  } else if (score >= 90) {
    rankBadge = 'S 顶尖级战术协同';
    rankColor = '#34c759';
    rankSub = '核心神装完备，被动技能联动强劲，战术威力与实战容错兼备';
  } else if (score >= 80) {
    rankBadge = 'A 优良级实战搭配';
    rankColor = '#0071e3';
  ...
  let burstBarPct = 10;
  if (isPhysicalHero) {
    burstBarPct = Math.min(100, Math.round((totalAd / 520) * 45 + (totalPhysPierce / 260) * 30 + (totalCrit / 70) * 15 + (hasSpellblade ? 10 : 0)));
  } else if (isMagicHero) {
    burstBarPct = Math.min(100, Math.round((totalAp / 1300) * 60 + (totalMagicPierce / 260) * 30 + (effNames.includes('博学者之怒') ? 10 : 0)));
  } else {
    burstBarPct = Math.min(100, Math.round((totalAd / 300) * 40 + (hasYellowShield ? 30 : 0) + (totalPhysPierce / 150) * 30));
  }
  burstBarPct = Math.max(10, Math.min(100, burstBarPct));

  const cdrBarPct = Math.max(8, Math.min(100, Math.round((cappedCdr / 40) * 100)));

  let kitingBarPct = Math.min(100, Math.round((bootsCount > 0 ? 30 : 8) + (totalPercentSpeed / 15) * 30 + ((hasSpellblade || hasIceHeart || hasBaoLie) ? 25 : 0) + (hasDash ? 15 : 0)));
  kitingBarPct = Math.max(10, Math.min(100, kitingBarPct));

  let defenseBarPct = Math.min(100, Math.round((totalBonusHp / 4200) * 40 + ((totalPdef + totalMdef) / 900) * 35 + ((hasDamageReduce || hasBloodRage || effNames.includes('魔女斗篷') || effNames.includes('不祥征兆')) ? 25 : 0)));
  defenseBarPct = Math.max(10, Math.min(100, defenseBarPct));

  let sustainBarPct = Math.min(100, Math.round((totalLifesteal / 25) * 40 + (totalSpellLifesteal / 25) * 40 + (hasPhoenix ? 30 : 0) + (hasYellowShield ? 20 : 0) + (hasBaZhe ? 20 : 0) + (hasSkillHeal ? 20 : 0)));
  if (isSustainGod) sustainBarPct = 100;
  sustainBarPct = Math.max(10, Math.min(100, sustainBarPct));
    ...
    <div class="synergy-bottom-grid">
      <div class="synergy-rating-card">
        ...
        <div class="synergy-score-circle">
          <span class="synergy-score-big" style="color: ${rankColor};">${score}</span>
          <div>
            <div style="font-size: 14px; font-weight: 700; color: ${rankColor};">${rankBadge}</div>
            <div style="font-size: 11px; color: var(--text-secondary); margin-top: 2px;">${rankSub}</div>
          </div>
        </div>
        <div class="synergy-radar-bars">
          ...
          <div class="synergy-radar-row">
            <span>技能周转 (CDR)</span>
            <div class="synergy-radar-track"><div class="synergy-radar-fill" style="width: ${cdrBarPct}%;"></div></div>
          </div>
          <div class="synergy-radar-row">
